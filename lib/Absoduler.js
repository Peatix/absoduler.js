(function () {
  ...
  Absoduler.prototype.Absodulerize = function(fallback) {
    if (this.isNode) {
      return this._absodulerizeNode();
    }
    else {
      return this._absodulerizeBrowser();
    }
  };

  Absoduler.prototype._absodulerizeNode = function() {
    this.constructor.NodeAbsodulerize.call(this,this.socket,this.serialize);
  };

  Absoduler.prototype._absodulerizeBrowser = function () {
    var that = this;
    ws.onmessage = function (wsmessage) {
      that.serialize.unpack(wsmessage);
      ...
      return that.handler(msg);
    };
    ws._abdl_original_send = ws.send;
    ws.send = function (wspayload, flags, callback) {
      ws._abdl_original_send(that.serialize.pack({ w: wspayload }));
    };
    ws.send_abdl = function (obj) {
      return ws._abdl_original_send(that.serialize.pack(obj));
    };
  };
